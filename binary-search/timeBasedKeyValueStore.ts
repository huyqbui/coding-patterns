...
class TimeMap {
  ...
  // Time: O(log n) due to running binSearch on the array in the cache at key
  // Space: O(1) ignoring the output result;
  get(key: string, timestamp: number) {
    let result = '';
    return this.binSearch(this.cache[key], timestamp, result);
  }

  binSearch(values: [string, number][], time: number, res: string) {
